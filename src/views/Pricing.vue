  ...
  <div class="flex justify-center">
    <div class="w-1/2 bg-blue-900 rounded-lg shadow px-6 py-12 flex flex-col items-center">
      <div class="bg-gray-400 rounded-full flex justify-around p-1">
        <button
          v-for="(price, frequency, id) in plans[0].pricing"
          :key="id"
      ...
      <div class="flex w-full pt-8">
        <div v-for="(plan, id) in plans" :key="id" class="text-white w-1/2">
          <h1 class="text-2xl font-bold">{{ plan.name }}</h1>
          <ul class="pt-4">
            <li v-for="(benefit, id) in plan.benefits[currentFrequency]" :key="id">{{ benefit }}</li>
          </ul>
